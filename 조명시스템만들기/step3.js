// Check : 전구(PointLight) 생성, 세기/색상 제어, 전구(mesh) 발광색(emissive) 동기화
// 전구 색을 바꾸면 스탠드 주변 색감‧전구 발광색이 같이 변합니다.
var createScene = function () {
    ...
    // === [NEW: 단계 3] 전구(PointLight)
    const pointLight = new BABYLON.PointLight("point", new BABYLON.Vector3(3.5, 1.45, 0.5), scene);
    pointLight.intensity = 2.5;
    pointLight.diffuse = BABYLON.Color3.FromHexString("#FFD7A6");
    pointLight.range = 15;
    // 전구 메시 발광색 동기화
    bulbMat.emissiveColor = BABYLON.Color3.FromHexString("#FFD7A6");
    ...
    panel.innerHTML = `
        ...
        <!-- [NEW: 단계 3] 전구(Point) --------------------------------->
        <div class="row"><label>전구 ON/OFF</label><input id="pointToggle" type="checkbox" checked /></div>
        <div class="row"><label>전구 세기</label><input id="pointIntensity" type="range" min="0" max="6" step="0.05" value="2.5" /></div>
        <div class="row"><label>전구 색</label><input id="pointColor" type="color" value="#ffd7a6" /></div>
    ...
    // === [NEW: 단계 3] 이벤트 ===================================
    $("pointToggle").onchange = e => pointLight.setEnabled(e.target.checked);
    $("pointIntensity").oninput = e => pointLight.intensity = parseFloat(e.target.value);
    $("pointColor").oninput = e => {
        const c = BABYLON.Color3.FromHexString(e.target.value);
        pointLight.diffuse = c; bulbMat.emissiveColor = c; // 전구 메시 발광색 동기화
    };
