class Playground {
  public static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {
    ...
    groundMat.albedoColor = BABYLON.Color3.FromHexString("#C8B8A8");
    ...
    wallMat.albedoColor = BABYLON.Color3.FromHexString("#F0F0F0");
    ...
    sofaMat.albedoColor = BABYLON.Color3.FromHexString("#707070");
    ...
    const sofa = BABYLON.Mesh.MergeMeshes(
        [sofaBase, sofaBack, sofaArmLeft, sofaArmRight],
    ...
    tableMat.albedoColor = BABYLON.Color3.FromHexString("#5C4033");
    ...
    lampMat.albedoColor = BABYLON.Color3.FromHexString("#D4AF37");
    ...
    const lampShade = BABYLON.MeshBuilder.CreateCylinder(
        "lampShade",
        { diameterTop: 0.7, diameterBottom: 1, height: 0.8, tessellation: 24 },
        scene
    );
    lampShade.position.set(4.5, 1.9, -2);
    const shadeMat = new BABYLON.PBRMaterial("shadeMat", scene);
    shadeMat.albedoColor = BABYLON.Color3.FromHexString("#F5F5DC");
    ...
    fixtureMat.albedoColor = BABYLON.Color3.FromHexString("#303030");
    fixtureMat.metallic = 1.0;
    fixtureMat.roughness = 0.5;
    fixtureBase.material = fixtureMat;

    const ceilingBulbs: BABYLON.Mesh[] = [];
    ...
    if (!document.getElementById("ui-style")) {
      ...
      style.textContent = `
        ...
        .toggle-btn {padding: 4px 8px; font-size: 11px; background: #2a3b52; border: 1px solid #3e5472; border-radius: 6px; cursor: pointer; color: #e9eef5;}
        .toggle-btn:hover {background: #3e5472;}
      `;
    ...
    panel.innerHTML = `
      <div class="title-bar">
        <h2>조명 컨트롤</h2>
        <button id="toggleControlsBtn" class="toggle-btn">숨기기</button>
      </div>
    ...
    ($("toggleControlsBtn") as HTMLButtonElement).onclick = () => {
      const container = document.getElementById("controls-container")!;
      const btn = document.getElementById("toggleControlsBtn")!;
      const isHidden = container.style.display === "none";
      container.style.display = isHidden ? "block" : "none";
      btn.textContent = isHidden ? "숨기기" : "보이기";
    };

    $("hemiToggle").onchange = e => hemiLight.setEnabled((e.target as HTMLInputElement).checked);
    $("hemiIntensity").oninput = e => hemiLight.intensity = parseFloat((e.target as HTMLInputElement).value);

    $("dirToggle").onchange = e => dirLight.setEnabled((e.target as HTMLInputElement).checked);
    $("dirIntensity").oninput = e => dirLight.intensity = parseFloat((e.target as HTMLInputElement).value);
    $("shadowToggle").onchange = e => ground.receiveShadows = (e.target as HTMLInputElement).checked;

    $("ceilingToggle").onchange = e => ceilingLight.setEnabled((e.target as HTMLInputElement).checked);
    $("ceilingIntensity").oninput = e => ceilingLight.intensity = parseFloat((e.target as HTMLInputElement).value);
    ...
    $("lampToggle").onchange = e => tableLampLight.setEnabled((e.target as HTMLInputElement).checked);
    $("lampIntensity").oninput = e => tableLampLight.intensity = parseFloat((e.target as HTMLInputElement).value);
    $("lampColor").oninput = e => {
      const c = BABYLON.Color3.FromHexString((e.target as HTMLInputElement).value);
    ...
    (document.getElementById("resetBtn") as HTMLAnchorElement).onclick = () => {
